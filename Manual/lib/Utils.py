...
class AnimatedHeatmap():
    ...
    def show(self, target_data):
        display(widgets.HBox([widgets.HTML('<h2> Current </h2>'), widgets.HTML('<h1> HeatMaps </h1>'), widgets.HTML('<h2> Target </h2>')], layout = widgets.Layout(justify_content = 'space-around', width = '100%')))
        scale = widgets.Image(
    ...
    def animate(self):
        while(self.active):
            if (self.animation_check == None or self.animation_check()):
